...
tmux send-keys "sudo docker exec -it ros-px4-humble /bin/bash -c \"cd ws; source /opt/ros/humble/setup.bash; source install/setup.bash; colcon build --packages-select fase_3; ros2 run fase_3 fase3_script; exec /bin/bash\"" C-m
...
tmux select-pane -t 4
tmux send-keys "sudo docker exec -it ros-px4-humble /bin/bash -c \"cd ws; source install/setup.bash; ros2 run ros_gz_image image_bridge /camera\"" C-m

# isso daqui é o script de pegar imagem. 
# tmux select-pane -t 5
# tmux send-keys "sudo docker exec -it ros-px4-humble /bin/bash -c \"cd ws/src/fase_3/fase_3; source /opt/ros/humble/setup.bash; source ../../../install/setup.bash; python3 get_image_feed.py\"" C-m
